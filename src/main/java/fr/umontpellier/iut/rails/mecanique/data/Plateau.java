...
import fr.umontpellier.iut.rails.mecanique.RouteMaritime;
import fr.umontpellier.iut.rails.mecanique.RoutePaire;
import fr.umontpellier.iut.rails.mecanique.RouteTerrestre;
...
public class Plateau {
    ...
    public List<IVille> getIPorts() {
        return villes.stream().filter(Ville::estPort).map(v -> (IVille) v).collect(Collectors.toList());
    }
    ...
    public List<IRoute> getIRoutes() {
        return routes.stream().map(v -> (IRoute) v).collect(Collectors.toList());
    }
